"""
messages.py — כל נוסחי ההודעות של הבוט במקום אחד
===================================================
רוצה לשנות נוסח? פשוט תגידי לי מה לשנות ואגע רק כאן.
לא נוגעים כאן בלוגיקה, תנאים, או הגדרות — רק טקסטים.

מבנה:
  SCENARIO_RESPONSES  — הודעה השנייה (msg2) לכל תסריט.
                        msg1 (הפיץ') מוגדר ב-simple_router._COMPANY_PITCH.
  ERROR_MSG           — הודעות שגיאה ומערכת

הערה על {{GREETING}}:
  בתסריט "greeting" בלבד משתמשים ב-{{GREETING}} כ-placeholder.
  הבוט מחליף אותו אוטומטית בברכה המתאימה לשעה:
  "בוקר טוב" / "צהריים טובים" / "ערב טוב" / "לילה טוב"
"""

# ─────────────────────────────────────────────────────────────────────────────
# הודעות תסריט — msg2 (ההודעה השנייה) לפי סוג הפנייה הראשונה
# ─────────────────────────────────────────────────────────────────────────────

SCENARIO_RESPONSES: dict[str, str] = {

    # ── ברכה כללית — לקוח שלח ברכה בלי כוונה ברורה ─────────────────────────
    # {{GREETING}} מוחלף אוטומטית בברכה לפי שעה (בוקר טוב / ערב טוב וכו')
    "greeting": "{{GREETING}}, איך אפשר לעזור לכם? 😊",

    # ── כתובת האולם ───────────────────────────────────────────────────────────
    "showroom_address": (
        "אולם התצוגה שלנו נמצא בבעלי המלאכה 15, נתיבות 📍\n"
        "שעות פעילות: א'–ה' 09:00–18:00 | ו' 09:00–13:00 | שבת סגור.\n"
        "מומלץ לתאם פגישת ייעוץ מראש — רוצים שאקבע לכם זמן?"
    ),

    # ── שעות פעילות ───────────────────────────────────────────────────────────
    "showroom_hours": (
        "שעות הפעילות: א'–ה' 09:00–18:00 | ו' 09:00–13:00 | שבת סגור 😊\n"
        "רוצים לתאם ביקור באולם התצוגה?"
    ),

    # ── תיקון / תקלה ──────────────────────────────────────────────────────────
    "repair": (
        "אוי, אני ממש מצטערת לשמוע 💙 בואו נטפל בזה מיד.\n"
        "מה בדיוק קורה עם הדלת?"
    ),

    # ── דלת ממ\"ד ──────────────────────────────────────────────────────────────
    "mamad": (
        "שמחה לעזור! האם מדובר בממ\"ד חדש, או שיש ממ\"ד קיים שרוצים להחליף את דלתו?"
    ),

    # ── פירוק מסגרת ───────────────────────────────────────────────────────────
    "frame_removal": (
        "אשמח להבין האם מדובר בדלת כניסה או דלת פנים, כדי שנוכל לכוון אתכם נכון."
    ),

    # ── דלתות מעוצבות (כללי) ─────────────────────────────────────────────────
    "designed_doors": (
        "כן, בהחלט, יש אצלנו מגוון דלתות מעוצבות ודגמים בסגנונות שונים.\n"
        "אשמח להבין האם אתם מתעניינים בדלת כניסה או דלת פנים, כדי שנוכל לכוון אתכם נכון."
    ),

    # ── פנייה מפורטת — דלת כניסה ─────────────────────────────────────────────
    # Step 1 of entrance queue: frame removal (always first — must precede style)
    "detailed_inquiry": (
        "האם מדובר בדלת כולל משקוף, או דלת בלבד?"
    ),

    # ── פנייה מפורטת — דלת פנים ──────────────────────────────────────────────
    "detailed_inquiry_interior": (
        "כדי שנוכל להכין עבורכם הצעת מחיר מדויקת — האם מדובר בהחלפה של דלתות פנים קיימות, "
        "בית בשיפוץ, או בית חדש ללא דלתות פנים כרגע?"
    ),

    # ── דלתות כניסה (גלישה) ───────────────────────────────────────────────────
    # Step 1 of entrance queue: frame removal (always first)
    "entrance_doors": (
        "האם מדובר בדלת כולל משקוף, או דלת בלבד?"
    ),

    # ── דלתות פנים (גלישה) ────────────────────────────────────────────────────
    "interior_doors": (
        "כן, בהחלט, יש אצלנו דלתות פנים במגוון סגנונות וצבעים.\n"
        "כדי שנוכל להכין עבורכם הצעת מחיר מדויקת — האם מדובר בהחלפה של דלתות פנים קיימות, "
        "בית בשיפוץ, או בית חדש ללא דלתות פנים כרגע?"
    ),

    # ── בקשת מחיר כללית ──────────────────────────────────────────────────────
    "vague_inquiry": (
        "בכיף! כדי שנוכל לתת לכם הצעת מחיר מסודרת, אשמח לדעת במה אתם מתעניינים — "
        "דלת כניסה, דלת פנים או משהו אחר?"
    ),

    # ── פנייה עמומה ───────────────────────────────────────────────────────────
    "ambiguous": (
        "אשמח להבין האם אתם מתעניינים בדלת כניסה או דלת פנים, כדי שנוכל לכוון אתכם נכון.\n"
        "אחרי שתכתבו את הפרט הזה, נמשיך יחד בצורה מסודרת."
    ),

    # ── בקשה לנציג אנושי ─────────────────────────────────────────────────────
    "human_request": (
        "בשמחה 😊 אסדר שיחזרו אליכם — מה השם ומספר הטלפון שלכם?"
    ),

    # ── חירום (פריצה / דלת שבורה דחוף) ──────────────────────────────────────
    "emergency": (
        "איזה סיוט 💙 קודם כל — הכל בסדר?\n"
        "אני מעבירה אתכם למנהל עכשיו — מה השם ומה המספר?"
    ),

    # ── קבלן / פרויקט גדול ────────────────────────────────────────────────────
    "contractor": (
        "פרויקט גדול — מנהל המכירות שלנו יטפל ישירות 🌟\n"
        "מה השם, טלפון ועיר שלכם?"
    ),

    # ── כיסוי גיאוגרפי ────────────────────────────────────────────────────────
    "geographic": (
        "אנחנו מגיעים לכל הארץ — דרום, שפלה, מרכז ועוד 😊\n"
        "מה סוג הדלת שמחפשים — כניסה, פנים, או משהו אחר?"
    ),

    # ── סטיקר בלבד ────────────────────────────────────────────────────────────
    "sticker_only": (
        "קיבלתי 😊 במה אפשר לעזור?"
    ),

    # ── אחריות ────────────────────────────────────────────────────────────────
    "warranty": (
        "אחריות מלאה על כל המוצרים וההתקנות שלנו — מעל שנתיים 😊\n"
        "יש משהו ספציפי שמדאיג אתכם, או שתרצו לשמוע על מוצר מסוים?"
    ),

    # ── זמן התקנה ─────────────────────────────────────────────────────────────
    "installation_time": (
        "זמן ההתקנה תלוי בלוח הזמנים ובפרטי הפרויקט 😊\n"
        "מנהל המכירות יסביר לכם בדיוק — מה הטלפון שלכם?"
    ),

    # ── צבעים ─────────────────────────────────────────────────────────────────
    "colors": (
        "יש לנו מגוון רחב של צבעים 😊\n"
        "דלתות כניסה: לבן, שמנת, אפור, שחור, וכל גוון ממניפת אלידור.\n"
        "דלתות פנים: לבן, שמנת, אפור, שחור, טורקיז ועוד.\n"
        "על איזה דלת מדובר?"
    ),
}


# ─────────────────────────────────────────────────────────────────────────────
# הודעות שגיאה ומערכת
# ─────────────────────────────────────────────────────────────────────────────

ERROR_MSG = {
    # מוצג ללקוח כשה-AI לא מצליח לענות (אחרי כל ניסיונות הגיבוי)
    "api_error": "רגע, בודקת 😊 תכתבו לי שוב בעוד רגע ואענה לכם",

    # מוצג ללקוח כשיש בעיית פיענוח תשובה פנימית
    "parse_error": "רגע, בודקת 😊 תכתבו לי שוב בעוד רגע ואענה לכם",
}
